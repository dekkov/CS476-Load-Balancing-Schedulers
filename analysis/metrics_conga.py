#!/usr/bin/env python3
"""metrics_conga.py — Parse FlowMonitor XML, spine trace CSV, and CONGA
log to compute load-balancing metrics for the CONGA simulation.

Usage:
    python3 metrics_conga.py <flowmon-conga.xml> <results_dir>
"""
...
def parse_conga_log(results_dir):
    """Parse conga-log.csv into a DataFrame."""
    csv_path = os.path.join(results_dir, "conga-log.csv")
    if not os.path.exists(csv_path):
        print("\nNo conga-log.csv found, skipping CONGA log analysis.")
        return None

    df = pd.read_csv(csv_path)
    if df.empty:
        print("\nCONGA log is empty (no flowlet reassignments).")
        return None

    print(f"\nCONGA log: {len(df)} flowlet reassignment(s)")
    for _, row in df.iterrows():
        print(
            f"  t={row['epoch_s']:.6f}s: {row['srcAddr']}:{row['srcPort']} -> "
            f"{row['dstAddr']}:{row['dstPort']}  "
            f"spine {int(row['oldSpine'])}->{int(row['newSpine'])}  "
            f"{row['rate_mbps']:.1f} Mbps  "
            f"CE: {row['oldCE']:.4f}->{row['newCE']:.4f}"
        )
...
def compute_conga_metrics(df):
    """Compute and print summary metrics for CONGA simulation."""
    mice = df[df["type"] == "mouse"]
    elephants = df[df["type"] == "elephant"]

    print("\n" + "=" * 60)
    print("CONGA SIMULATION RESULTS")
    print("=" * 60)
...
def generate_conga_plots(mice, elephants, spine_df, conga_df, results_dir):
    """Generate plots for CONGA simulation results."""
    n_panels = 4 if conga_df is not None else 3
    fig, axes = plt.subplots(1, n_panels, figsize=(6 * n_panels, 5))

    # Panel 1: Elephant throughput
    ax = axes[0]
    if not elephants.empty and "throughput_mbps" in elephants.columns:
        ax.bar(range(len(elephants)), elephants["throughput_mbps"].values)
    ...
    # Panel 4: CONGA flowlet reassignment timeline
    if conga_df is not None and n_panels == 4:
        ax = axes[3]
        if not conga_df.empty:
            colors = ["#1f77b4", "#ff7f0e"]
            for _, row in conga_df.iterrows():
            ...
            ax.set_title("CONGA Flowlet Reassignments")
        else:
            ax.text(0.5, 0.5, "No reassignments", ha="center", va="center",
                    transform=ax.transAxes)
            ax.set_title("CONGA Reassignments")

    plt.tight_layout(pad=2.0)
    plot_path = os.path.join(results_dir, "conga-metrics.png")
...
def main():
    if len(sys.argv) < 3:
        print(f"Usage: {sys.argv[0]} <flowmon-conga.xml> <results_dir>")
        sys.exit(1)
    ...
    mice, elephants = compute_conga_metrics(df)
    spine_df = analyze_spine_trace(results_dir)
    conga_df = parse_conga_log(results_dir)
    generate_conga_plots(mice, elephants, spine_df, conga_df, results_dir)
